...
from erlking import _core_computation, _bias_correction


def test_core_computation():
    inbag = np.array([[1.,  2.,  0.,  1.],
                      [1.,  0.,  2.,  0.],
                      [1.,  1.,  1.,  2.]])
    ...
    pred_centered = np.array([[-20, -20, 10, 30],
                              [-20, 30, -20, 10]])
    n_trees = 4

    our_vij = _core_computation(X_train, X_test, inbag,
                                pred_centered, n_trees)
...
def test_bias_correction():
    ...
    our_vij = _core_computation(X_train, X_test,
                                inbag, pred_centered, n_trees)
    our_vij_unbiased = _bias_correction(our_vij, inbag,
                                        pred_centered, n_trees)
